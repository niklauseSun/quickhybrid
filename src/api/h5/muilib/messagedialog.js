...
const createInput = (placeholder) => {
  const inputHtml = `<input type="text" autofocus placeholder="${
    placeholder || ''
  }"/>`;
...
const createInner = (message, title, extra) => {
  ...
  return `<div class="${CLASS_POPUP_INNER}">${divPopTitle}${divPopText}${
    extra || ''
  }</div>`;
};
const createButtons = (btnArray) => {
  ...
  for (let i = 0; i < len; i += 1) {
    ...
    btns.push(
      `<span class="${CLASS_POPUP_BUTTON} ${classBold}">${btnArray[i]}</span>`
    );
  }
...
export function prompt(params, success) {
  ...
  const innerHtml = createInner(
    options.text,
    options.title,
    createInput(options.hint)
  );
  const buttonHtml = createButtons(options.buttonLabels);
